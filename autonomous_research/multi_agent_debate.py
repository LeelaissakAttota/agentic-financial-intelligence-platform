...
class MultiAgentDebate:
    ...
    async def _conduct_round(
        ...
        elif round_num == max(2, round_num - 1):
            phase = DebatePhase.REBUTTAL
        elif round_num == max(3, round_num):
    ...
    async def _generate_proposer_argument(
        ...
        if round_num == 1:
            ...
            return (
                ...
                f"Addressing challenges: {len(challenges)} points raised. "
    ...
    def _extract_agreements_disagreements(
        ...
        for round_obj in rounds:
            # Look for explicit agreements in responses
            for participant, response in round_obj.responses.items():
                if "agree" in response.lower() or "concede" in response.lower():
                    agreements.append(f"{participant} agreed in round {round_obj.round_number}")
                if "disagree" in response.lower() or "however" in response.lower():
                    disagreements.append(f"{participant} disagreed in round {round_obj.round_number}")
    ...
    def _extract_dissenting_views(self, positions: Dict[str, DebatePosition]) -> List[str]:
        ...
        for pid, pos in positions.items():
            if pos.stance == "oppose":
                dissenting.append(f"{pid} ({pos.role.value}): {'; '.join(pos.key_arguments[:2])}")
        return dissenting
    ...
    def _synthesize_consensus(
        ...
        """Synthesize consensus thesis."""
        # Start with original thesis
        consensus = original_thesis
        
        # Incorporate agreed modifications
        agreements = [a for p in positions.values() for a in p.key_arguments if "agree" in a.lower()]
        
        if agreements:
            consensus += f" [Consensus modifications: {'; '.join(agreements[:3])}]"
        
        return consensus
    
    def get_debate(self, debate_id: str) -> Optional[DebateResult]:
        """Get a completed debate."""
        return self._debates.get(debate_id)
    
    def get_stats(self) -> Dict[str, Any]:
        return {
            "completed_debates": len(self._debates),
...
async def close_multi_agent_debate() -> None:
    global _multi_agent_debate
